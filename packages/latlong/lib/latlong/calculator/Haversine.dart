...
class Haversine implements DistanceCalculator {
    ...
    double distance(final LatLng p1, final LatLng p2) {
        final double sinDLat = math.sin((p2.latitudeInRad - p1.latitudeInRad) / 2);
        final double sinDLng = math.sin((p2.longitudeInRad - p1.longitudeInRad) / 2);

        // Sides
        final double a = sinDLat * sinDLat + sinDLng * sinDLng * math.cos(p1.latitudeInRad) * math.cos(p2.latitudeInRad);
