  ...
  dynamic clickToNextCheck(User user, Function _closeCheckoutContainer,
    ...
    if (viewNo < maxViewNo) {
      if (viewNo == 3) {
        ...
        if (checkout1ViewState.userEmailController.text.isEmpty) {
          ...
          showDialog<dynamic>(
              ...
              builder: (BuildContext context) {
                return ErrorDialog(
                  message: Utils.getString(
                      context, 'warning_dialog__shipping_email'),
                );
              });
        ...
        if (checkout2ViewState.shippingMethodProvider.psValueHolder
                    .standardShippingEnable ==
                PsConst.ONE &&
            checkout2ViewState.shippingMethodProvider.selectedShippingMethod ==
                null &&
            (checkout2ViewState.shippingMethodProvider.defaultShippingId ==
                    '0' ||
                checkout2ViewState.shippingMethodProvider.defaultShippingId ==
                    '')) {
          showDialog<dynamic>(
              ...
              builder: (BuildContext context) {
                return ErrorDialog(
                  message: Utils.getString(
                      context, 'warning_dialog__shipping_method'),
                );
              });
        } else {
          viewNo++;
