...
class Path<T extends LatLng> {
    ...
    Path.from(final Iterable<T> coordinates, { final LatLngFactory factory = _defaultLatLngFactory })
        : _coordinates = List<T>.from(coordinates), _latLngFactory = factory {
    ...
    /// we need two
    Path<T> equalize(final num distanceInMeterPerTime,{ final bool smoothPath = true }) {
        ...
        if(baseLength == stepDistance) {
            return Path<T>.from(<T>[ _coordinates.first, _coordinates.last ]);
        }

        final List<T> tempCoordinates = List<dynamic>.from(_coordinates);
        final Path<T> path = Path<T>();
    ...
    ///
    num get distance {
        final List<T> tempCoordinates = List<T>.from(_coordinates);
        double length = 0.0;
    ...
    CatmullRomSpline2D<double> _createSpline(final LatLng p0,final LatLng p1,final LatLng p2,final LatLng p3) {
        ...
        return CatmullRomSpline2D<double>(
            Point2D<double>(p0.latitude,p0.longitude),
            Point2D<double>(p1.latitude,p1.longitude),
            Point2D<double>(p2.latitude,p2.longitude),
            Point2D<double>(p3.latitude,p3.longitude)
        );
    }

    /// Convert [Point2D] to [LatLng]
    LatLng _pointToLatLng(final Point2D<num> point) => _latLngFactory(point.x,point.y);
}
